...
const programs = [
  ...
  },
  {
    ...
    subtitle: '5 años',
    description:
      'Programa de transición que prepara a los niños para el éxito en la educación primaria con bases sólidas.',
    image: kinderImage,
  ...
  },
  {
    ...
    description:
      'Formación integral con énfasis en el desarrollo académico, valores cristianos y habilidades para la vida.',
    image: primariaImage,
...
const subjects = [
  ...
  { icon: Leaf, name: 'Ciencias Naturales', color: 'text-green-500' },
  { icon: Globe, name: 'Ciencias Sociales', color: 'text-green-500' },
  { icon: Languages, name: 'Inglés', color: 'text-sky-300' },
  { icon: Palette, name: 'Arte', color: 'text-purple-500' },
  { icon: Laptop, name: 'Tecnología', color: 'text-pink-500' },
  { icon: Bot, name: 'Robótica', color: 'text-cyan-300' },
  { icon: Lightbulb, name: 'Emprendurismo', color: 'text-yellow-300' },
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground mb-8">
              Desde preescolar hasta primaria, ofrecemos programas diseñados para desarrollar el potencial único de
              cada niño en un ambiente de amor y excelencia.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {programs.map((program, index) => (
        <Section key={program.id} id={program.id} className={index % 2 === 1 ? 'bg-secondary/30' : ''}>
          <div
          ...
          >
            ...
            >
              <span
                className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-4 ${program.color} text-foreground`}
              >
                {program.subtitle}
              </span>

              <h2 className="font-heading font-bold text-3xl lg:text-4xl text-foreground mb-4">{program.title}</h2>

              <p className="text-muted-foreground text-lg mb-6 leading-relaxed">{program.description}</p>
      ...
      <Section className="bg-primary">
        {/* ✅ Todo blanco, mismo tono */}
        <div className="text-center">
          <h2 className="font-heading font-bold text-3xl lg:text-4xl text-primary-foreground">
            Áreas de aprendizaje
          </h2>
          <p className="mt-2 text-primary-foreground text-base">
            Currículo integral que desarrolla todas las dimensiones del estudiante.
          </p>
        </div>

        <div className="mt-8">
          <p className="text-center text-primary-foreground mb-8 max-w-2xl mx-auto">
            Nuestro currículo está alineado con los estándares del MEDUCA y enriquecido con programas complementarios
            de inglés, arte y formación en valores.
          </p>
        </div>

        {/* ✅ 10 tarjetas: responsive para que no se apriete */}
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-4">
          {subjects.map((subject, index) => (
            <motion.div
              key={subject.name}
              className="bg-primary-foreground/10 backdrop-blur rounded-xl p-6 text-center min-h-[120px] flex flex-col items-center justify-center"
            ...
            >
              ...
              <p className="font-semibold text-primary-foreground text-sm leading-tight">{subject.name}</p>
            </motion.div>
          ))}
        </div>
      </Section>
